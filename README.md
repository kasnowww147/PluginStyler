...
**PluginStyler** is a specialized Windows utility designed for music producers using **Bitwig Studio**, **Ableton Live**, and **Cockos Reaper**.
...
* **🎨 Dynamic Coloring**
    ...
    * **Rainbow Mode:** Smooth, threaded RGB color cycling for window borders with adjustable saturation (Zero UI lag).
...
* **⚙️ Architecture & Performance**
    * **Smart Window Filtering:** Intelligently targets plugin containers (e.g., Bitwig `pluginhost`, Reaper `VST` windows) while strictly excluding main DAW interfaces to prevent conflicts.
    * **Zero Audio Interference:** Utilizes **PID caching with TTL** and **background threading** to ensure **negligible (~0%) CPU impact** on real-time audio processing.
...
* **Supported DAWs:** Bitwig Studio, Ableton Live, Cockos Reaper.
...
**PluginStyler** 是一款专为 **Bitwig Studio**、**Ableton Live** 和 **Cockos Reaper** 用户设计的 Windows 窗口美化工具。
...
* **🎨 动态色彩管理**
    ...
    * **RGB 彩虹模式：** 采用独立线程渲染的动态 RGB 颜色循环边框，色彩饱和度可调，且**完全不卡顿 UI**。
    * **色板管理：** 保存并管理您常用的自定义颜色配置。
* **👻 透明度控制**
    * 独立控制激活与非激活窗口的透明度。
    * 支持非焦点窗口半透明，提升多任务工作流的可视性。
* **⚙️ 架构与性能**
    * **智能过滤：** 精确识别插件宿主进程（如 Bitwig 的 `pluginhost` 或 Reaper 的 `VST` 窗口），同时严格排除 DAW 主窗口。
    * **零音频干扰：** 采用 **PID 智能缓存 (TTL机制)** 和 **后台异步处理**，大幅降低系统调用频率，确保对实时音频处理 **近乎 0% 的 CPU 占用**。
...
* **支持的 DAW：** Bitwig Studio, Ableton Live, Cockos Reaper。
